...
def lconvista(config, shrinkage):
    ...
    Z = nconv(-X , D[:,:,::-1,::-1].dimshuffle(1,0,2,3), border_mode='full', image_shape=config['Xshape'], filter_shape=fs2)#T.zeros(config['Zinit'],dtype=theano.config.floatX)
    Z1 = Z
    def rec_error(X,Z,D):
    ...
    for i in range(layers):
        gradZ = nconv(nconv(Z,D,border_mode='valid',image_shape=config['Zinit'],filter_shape=fs1) - X , D[:,:,::-1,::-1].dimshuffle(1,0,2,3), border_mode='full', image_shape=config['Xshape'], filter_shape=fs2)
    ...
    sparsity = 0#T.mean(theano.map(lambda Zi: T.sum(T.abs_(Zi)),Z)[0])#alternative: mean also over filter index dimension (axis 1), not just axis 0
    cost = rec_error(X,Z,D) + sp_lmbd * sparsity

    return X, params, cost, Z, L
...
if __name__ == '__main__':
    ...
    config = {"D": D, "theta": theta, "L": L, "lambda": lmbd, "Zinit": zinit_shape, "layers": layers, "Xshape": Xshape}
    ...
    x, params, cost, z, gradZ = lconvista(config=config, shrinkage=crino.sh)
    ...
    #train = theano.function([x], gradZ  , updates=updates, allow_input_downcast=True)
    ...
    for epoch in xrange(epochs):
        ...
        for mbi in xrange(batches):
            #print np.sum(train(data[mbi*btsz:(mbi+1)*btsz])**2)
            #print '>>>>>>>>>>>>>>>>>'   
